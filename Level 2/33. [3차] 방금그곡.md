jihyukim
```js

```

jabae
```js
function solution(m, musicinfos) {
    let [time, title] = [0, ''];
    const neo = getFullNote(m, m.length - (m.split('#').length - 1));

    musicinfos.forEach((el) => {
        let [s, e, tt, n] = el.split(',');
        s = s.split(':'), e = e.split(':');
        const t1 = new Date(2022,12,30,s[0],s[1]).getTime();
        const t2 = new Date(2022,12,30,e[0],e[1]).getTime();
        const t = (t2 - t1) / 1000 / 60;
        const real_n = getFullNote(n, t);

        if (!checkIncludes(real_n, neo)) return ;
        if (time < t) time = t, title = tt;
    })
    return !time ? '(None)' : title;
}

function checkIncludes (full, find) {
    for (let i = 0; i <= full.length - find.length; i++) {
        if (full[i] === find[0]) {
            for (let j = 0; j < find.length; j++) {
                if (full[i + j] !== find[j]) break ;
                else if (j === find.length - 1) return true;
            }
        }
    }
    return false;
}

function getFullNote (note, time) {
    const original = note.split('').reduce((acc, cur) => {
        cur === '#' ? acc[acc.length - 1] += '#' : acc.push(cur)
        return acc;
    }, []);
    if (time > original.length) for (let i = 0; i <= time; i = original.length) original.push(...original);
    return original.slice(0, time);
}
```

daekim
```js
function solution(m, musicinfos) {
    let arr = musicinfos.map(el => el.split(',').map((e, i) => {
        if (e.includes(':')) e = Number(e.split(":")[0] * 60) + Number(e.split(":")[1])
        else if (i === 3) e = e.split('');
        return e
    }));
    
    let radio = [];
    arr.forEach((e, idx) => {
        let music = [];
        let note = '';
        let len = e[3].length;
        let cnt = 0;
        for (let i = 0; i < e[1] - e[0] + cnt; i++){
            if (e[3][i % len] !== '#'){
                note = e[3][(i + 1) % len] !== '#' ? e[3][i % len] : (e[3][i % len] + '#') ;
                music.push(note);
            } else cnt++;
        }
        radio.push([e[2], music.join(''), idx, e[1] - e[0]]);
    })

    let long = 0;
    let first = 24 * 60;
    let tmp = -1;
    let idx = -1;
    for (let i = 0; i < radio.length;) {
        tmp = radio[i][1].indexOf(m, tmp + 1);
        if (tmp === -1) {
            i++;
            continue ;
        }
        if (tmp > -1 && radio[i][1][tmp + m.length] !== '#') {
            if (long < radio[i][3]) {
                long = radio[i][3]
                first = radio[i][2]
                idx = i;
            }
            else if (long === radio[i][3]) {
                if (first > radio[i][2]) {
                    first = radio[i][2]
                    idx = i;
                }
            }
            i++;
            tmp = -1;
        }
    }
    return idx === -1 ? "(None)" : radio[idx][0]
}
```
